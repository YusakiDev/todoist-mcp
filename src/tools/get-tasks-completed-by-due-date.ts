...
export function registerGetTasksCompletedByDueDate(server: McpServer, api: TodoistApi) {
    server.tool(
        ...
        {
            since: z.string().describe('Start due date for completed tasks (YYYY-MM-DD format)'),
            until: z.string().describe('End due date for completed tasks (YYYY-MM-DD format)'),
